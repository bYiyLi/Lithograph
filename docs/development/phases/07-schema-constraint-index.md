...
- [ ] Graph View 隐藏一个已占用 KEY/UNIQUE value 的 element 时，view 内 CREATE/MERGE 仍由完整图 constraint 检测冲突并返回 `CONSTRAINT_ERROR`，不产生 view-local uniqueness；
...
- [ ] 同一 Graph View 下 lookup/range/text/point index seek 与 view-aware scan result 相同，hidden index candidate 不泄漏；
- [ ] Graph Type / Constraint / Index DDL 与 `graphView` 同时提交时返回 `INVALID_ARGUMENT`，不产生 Schema Commit；

...
适用 v0.1.0。首先区分“哪些数据一起落盘”和“历史中形成几个 Commit”。它们不是同一个问题。
...
| Native tx_begin / tx_execute / tx_commit | 任一 execution 失败使整个事务 abort | 有成功 mutation 时恰好 1；只有读取时 0 |
...
最后只有 `one`、`two`。普通查询失败会撤销该 invocation 的写入；外层事务后续提交还是回滚由宿主决定，不能把 SQL Bridge 的单次失败等同于 Native explicit transaction 的整体自动 abort。

使用 Python 时明确设置事务策略。示例使用 `isolation_level=None`，SQL BEGIN/COMMIT 由应用显式控制。不要依赖不同 binding 的默认 implicit transaction 行为。

## 多次执行只产生一个 Commit

只有 Native explicit transaction 提供以下生命周期：
...
这里是生命周期说明，真实 C 函数接收 UTF-8 指针、长度及输出指针，完整签名见 [Native API](../reference/native-api.md)。SQL Bridge 没有 `tx_begin` SQL 函数，也没有在 SQL 事务末尾“自动 squash”的配置。

begin 必须在 SQLite autocommit、没有 active Native transaction 时运行；`expectedHead` 可要求当前 Branch head 等于已知 Commit。begin 取得 single-writer ownership，后续 execution 看到此前 staged writes。Constraint 按 statement 即时验证，不自动延迟到最后。
...
显式取消使用 tx_abort。没有 active transaction 时调用 execute/commit/abort 返回 `INVALID_ARGUMENT` + `SQLITE_MISUSE`。清理失败为 `INTERNAL_ERROR` 时关闭并丢弃 connection，不把它放回连接池。
...
`BUSY` 可采用有上限的等待/退避，但先判断调用是否已经成功提交，不能对任意写入盲目重试。`BRANCH_HEAD_MOVED` 需要重新读取最新 head 并重做业务决策，不会自动 merge。跨线程共享 connection 遵循宿主 SQLite threading mode；连接池不能把一个 active Native transaction 分给其他请求。
...
可执行单 Commit 示例见 [native_transaction.c](examples/native_transaction.c)。

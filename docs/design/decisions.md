...
- 取舍：stock SQL console 不能直接新增顶层 `MATCH` grammar，因此通过 SQLite SQL function / table-valued function 输入 Cypher。

<a id="d2"></a>

### D2 Rust Core

- 决定：Rust 负责 Engine；C ABI 只出现在 SQLite loadable-extension boundary 与 Embedding Provider SPI，不再提供 application-facing Cypher binding ABI。
- 依据：需要 native extension、内存安全、复杂 parser/executor、并发与版本数据结构。
- 备选：纯 C/C++；直接延续 GraphQLite C architecture。
- 取舍：需要维护 SQLite/Provider FFI wrapper，但 Application 只依赖标准 SQLite driver，不需要同时绑定 Lithograph 私有 query C API。
...
### D8 SQLite SQL 是完整 Application Execution Surface

- 决定：Application-facing Cypher execution 只提供 SQLite SQL surface；`lithograph()` 返回完整 envelope，`lithograph_rows()` 返回真正的 `columns -> row* -> summary` execution stream，两者共享同一个 parser/planner/executor 与 query capability。普通 autocommit SQL execution直接支持 `CALL { ... } IN TRANSACTIONS`；不保留 Native query ABI。
- 依据：Phase 14 的真实 SQLite acceptance 已证明 SQL **scalar callback** 可以在 host `sqlite3*` 上建立、提交和回滚 Engine-owned transaction，因此此前“SQL callback 天生不能拥有 transaction boundary”的绝对假设不成立；但历史 rows 一直是 read-only，没有证明 virtual-table cursor 内可以跨 `xNext` 生命周期在同一 host connection 持有普通 write SAVEPOINT，也没有证明单次 scalar/vtab execution 内 transaction-owning query 所需的重复 inner transaction boundary。本决定因此要求 Phase 15 把这两项都作为 minimum/current SQLite 的 same-connection feasibility gate；任一未通过都必须回到本设计，不能偷换成 sibling graph connection、SQLite fork 或隐藏 Native query path。同时双 SQL/Native API 会迫使普通 database client 额外持有 `sqlite3*` 与私有 ABI，增加重复 adapter、测试和发布面。
- 备选：继续把 SQL rows 限制为 read-only；保留 Native 完整执行面；让 application 自行解析 Cypher 分类后选择 adapter。
- 取舍：SQL virtual-table cursor 必须承担 write/external-I/O/cancel/transaction-owning execution 的完整生命周期，并证明多 scan、多 batch、failure 与 early-close 正确；换取单一 application integration surface 与不分叉的 Cypher capability。

<a id="d9"></a>

### D9 Graph View 是 Execution Context，不扩展 Cypher

- 决定：通过 SQLite execution surface 的 `options.graphView` 选择 query-local Property Subgraph；selector 在一次 execution 内固定，visibility 随 Cypher 当前 clause graph state 计算；完整 versioned Schema / Constraint 不被 view 投影。Cypher text、AST 与 `CY25-2026.08` grammar 不增加 Lithograph-specific syntax。
...
- 决定：以 `sqlite3*` connection 作为 transaction identity，提供 SQL `lithograph_tx_begin -> (lithograph | lithograph_rows)* -> lithograph_tx_commit | lithograph_tx_abort` lifecycle，不增加 opaque transaction handle，也不提供 `lithograph_tx_execute()`。多个标准 Cypher current-graph execution 共享 transaction-local staged graph/Schema/Index state，成功 transaction 只创建一个最终 Layer/Commit 并只移动 Branch 一次。caller-owned SQLite transaction 继续只负责 durability atomicity，Cypher `IN TRANSACTIONS` 继续按 batch 独立 Commit，二者都不替代 explicit transaction。
- 依据：通用数据库调用方存在一个逻辑变化需要跨多个 query 读取中间结果、分配 identity、修改 graph + Schema/Constraint/Index，但版本历史只应出现最终一致 Snapshot 的需求。既然 `lithograph()` / `lithograph_rows()` 已经是完整 execution surface，再保留专用 tx-execute 会重复同一能力；connection-local active state 足以让普通 SQL execution 自动加入 staged transaction。
- 备选：保留 `lithograph_tx_execute()`；只允许“一条 Cypher query -> 一个 Commit”；用 caller-owned SQLite transaction 包裹多个 Commit；要求调用方构造 Structural Patch；完成后自动 squash/rewrite history；建立独立 server/session transaction layer。
...
- 备选：一次性 merge + 全量 resolutions；长生命周期 SQLite explicit transaction；merge prepare/finalize 但 candidate 只存内存；finalize-time application callback；让上层先 merge 再 revert invalid result。
...
- 依据：TEMP Standard Index 在 connection 关闭后丢失，且 cache identity 按 Snapshot 分裂；持久化必须同时尊重 reserved-schema、普通 read/read-only connection 不因 cache miss 隐式写 `main`、staged state 与版本类型语义。
...
- 决定：保留 Cypher 25 Raw Vector Property/Index/`SEARCH` 全部语义，另加一个非 grammar 的 `db.index.semantic.*` managed surface。Semantic source 是一个 String Property，派生 Vector 只存在于 derived materialization；具体 Embedding 由同 connection 上的普通 SQLite loadable extension 按 `EmbeddingProviderV1` 提供，Lithograph Core 不内置 vendor/model runtime。text->Vector result cache 同样属于具体 Provider；Lithograph 不保存 persistent/LRU embedding result cache、不为它升级 storage format，也不暴露统一 cache maintenance procedure。OpenAI-compatible Provider 可按 versioned `providerConfig.cache` 使用独立 SQLite cache DB。
...
- 取舍：产品存在 Raw Vector 与 Managed Semantic 两个明确入口，首次 Semantic query 可能产生 external-I/O cost，历史 bit-identical rebuild 依赖部署固定 Provider runtime；不同 Provider 可以采用不同 cache policy，Lithograph 不再统一观察 cache stats/lifecycle。换取的是标准 Cypher Vector 兼容不被破坏、任意正常 Cypher text mutation 不会留下 stale hidden vector、Provider cache 与 graph/history storage 解耦、只读 Lithograph database 也不会因 embedding cache 产生 operational write。需要 Vector 本身成为历史真源或完全可复现时继续使用 Raw Vector。

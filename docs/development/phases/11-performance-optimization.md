...
Phase10保持 `done`，其原始证据作为历史保留。后续调查发现的真实access-path问题由本Phase修复，不能用旧checklist勾选或logical operator名称作为免修理由。产品行为唯一真源仍为 [docs/design.md](../../design.md)；本计划只定义实施顺序、Feature owner与验证。
...
**优先级：P0；来源：Design §17.1–17.3；依赖：Phase10。**
...
**优先级：P0；来源：Design §8.3.1；依赖：11.1。**
...
**优先级：P0；来源：Design §11.7.1、§11.7.3–11.7.4、§14.3.1；依赖：11.3。**
...
`lithograph.index.rebuild`完全按Design §11.7.3执行，不新增并行public配置体系。固定table/index inventory由同一schema声明验证；generation build不全局DROP其它index。NewDDL和Native staged index在正确Commit/durability边界发布。
...
**优先级：P0；来源：Design §11.4、§11.7.2；依赖：11.4。**

在persistent base之上实现target-state changed-owner overlay，不复制全index、不为每个Commit建立全generation。已有generation的Relationship property读取使用其相关type/domain access path；初次完整构建与Lookup复用按Design §11.7.3执行并独立计时，不能把全库枚举带入ready read。
...
- 首行前cache失效的fallback与已输出结果后的fail-closed分开测试，遵守Design §11.7.4；不得重复输出或在错误之后发送成功SUMMARY。离线单entry篡改由显式integrity oracle覆盖，不把未做的全量核验称为point query已验证。

### Feature 11.6 剩余executor热点

**优先级：P1；来源：Design §7.8、§17.3；依赖：11.5。**
...
**优先级：P1；来源：Design §17.2–17.3；依赖：11.6。**
...
**优先级：P0；来源：Design §14、§17；依赖：11.1–11.7。**
...
| --- | --- | --- | --- | --- | --- |
| P11-MEASURE | quality | 可重现、分缓存/adapter的计时与物理work | §17.1 | P0 | 11.1、6.1/6.2 |
| P11-ADJ | functional | 同序keyset与完整overlay邻接语义 | §8.3.1 | P0 | 11.2、6.2 |
| P11-STATE | data | query-owned state/read guard与revision | §7.8 | P0 | 11.3、并发/清理矩阵 |
| P11-CACHE | data | generation reuse、delta覆盖、可重建 | §11.7 | P0 | 11.4/11.5 |
| P11-API | interface | rebuild procedure/adapters/errors | §11.7.3 | P0 | procedure inventory + SQL/Native |
| P11-FORMAT | operational | 显式format3迁移与旧历史保持 | §14.3.1 | P0 | 11.4/11.8 recovery/migration |
| P11-SAFETY | security | 无隐式read写入、schema/TEMP-trigger防线 | §4.1、§11.7.3 | P0 | readonly/authorizer/collision tests |
| P11-LATENCY | quality | 定量延迟、RSS、非回退目标 | §17.2 | P0 | 11.8、6.3 |
| P11-STRESS | quality | Search质量、并发与资源回收 | §17.3 | P1 | 11.7/11.8 |
...
| Search | Design §17.3的100K/1M文档与向量集合 | build/read/cold/warm、recall与visible exact oracle |
...
- 固定degree/result下，汇总每个query的全部相关SQLite statement，满足Design §17.1的无关数据增长比值gate；同时验证covering endpoint/type path和无不必要SORT。NodeID的B-tree point work允许对数变化，不把不含endpoint条件的主键范围扫描叫seek通过。
...
完整采用Design §17.2的统一hardware、重复次数、每状态P50/P95、长query中位数/最大值、latency/RSS与非回退要求，不在本计划重复维护另一份数值表。Before/after共用query/fixture/adapter/runtime/durability配置；generation存在与否必须标记，完整build和reopen成本同时保存。

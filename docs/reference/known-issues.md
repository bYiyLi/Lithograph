...
这些是编写手册时在**实际已发布 macOS arm64 制品**上复现的实现差异，不是新的设计要求，也不表示已在某个未发布版本修复。当前文档任务不修改引擎代码、已有 tag 或发布资产。
...
上述发布问题均有明确规避路径；这不代表已经修复。后续发布若修复，应针对新制品重新验证、更新该版本文档，不把 v0.1.0 的已知事实静默抹掉。

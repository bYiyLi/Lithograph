...
- Phase 11：`done`；
- Phase 12：`ready`；
- `docs/development/cypher25-compatibility.md` 保留 Phase 00–11 已执行证据；Full-text family 因 query-time analyzer 组合语义缺口当前为 `partial`，修复 owner 是 Phase 12；新的 provider supplemental inventory 为 `planned`。Phase 历史完成状态与当前 capability 缺口分开记录，不用旧的 `done` 掩盖待修复项。
...
| [12 Full-text / FTS5 Tokenizer 扩展](phases/12-fulltext-tokenizer.md) | `ready` | 原生 tokenizer specification、Schema/历史/cache、query analyzer 与真实 SQLite 扩展验收 | 08、09、11 |
...
以下保留Phase00–10的首个功能版本完成标准。Phase11另需满足其性能验收与Design §17.1–17.3，Phase12另需满足其FTS5 provider验收与Design §11.5；不能以基础功能版本已经完成代替后续专项完成。

...
- [ ] format `1 -> 2` migration 新增 Commit Data / Tag / Merge Session operational storage，migration 前后既有 Commit ID、Snapshot hash 与 history semantics 完全一致，失败原子 rollback；升级后的 sidecar/ref/session 初始集合为空；
...
- [ ] explicit transaction 内 Version Procedure、checkout/GC 与 `CALL ... IN TRANSACTIONS` / `IN CONCURRENT TRANSACTIONS` 返回 `TRANSACTION_BOUNDARY_REQUIRED`；per-execution `branch/at/author/message/mergeSession` 返回 `INVALID_ARGUMENT`；
...
- [ ] `merge.start` pin target/source 为 immutable `ours/theirs`，创建 Session 后无论 `up_to_date`、`fast_forward` 还是 diverged 都不立即创建 Commit/移动 Branch；
- [ ] `merge.start` 在 Session 持久化的短 write transaction 内重新确认 pinned ours/theirs 仍存在，再原子建立新的 GC root；与 concurrent GC 竞争时不得留下引用已删除 Commit 的 Session，Commit 已被 GC 时返回 `VERSION_NOT_FOUND` 且无 partial workspace；
- [ ] `merge.start(source, expectedHead)` 提供 expectedHead 时，在与 Session row 建立相同的 writer boundary 内比较 target Branch 当前 head；mismatch 返回 `BRANCH_HEAD_MOVED` 且没有 Session/resolution/GC-root side effect，match 时返回 Session 的 `ours == expectedHead`；省略时继续允许创建基于调用开始所 pin `ours` 的 Session；
- [ ] Merge Session procedure option boundary 与 Design 一致：只有 `merge.start` 接受 query-level `branch`，只有 `merge.finalize` 接受 `author/message`；get/list/conflicts/resolve/abort 不接受 branch/author/message，任何 version mutation 与 `at`、任何 Version Procedure 与 `graphView` 的既有拒绝规则继续成立；
- [ ] Merge Session error precedence 与 Design 一致：不存在 Session -> `MERGE_SESSION_NOT_FOUND`；stale expected revision/cursor/candidate revision -> `MERGE_SESSION_CHANGED`；当前 revision unresolved -> `MERGE_CONFLICT`；finalize target Branch 不存在 -> `BRANCH_NOT_FOUND`、head 不等 -> `BRANCH_HEAD_MOVED`；这些 boundary error 都没有 Commit/ref/session-resolution 副作用；
- [ ] source 已包含于 target 时 Session `status=up_to_date`；target 是 source ancestor 时 `status=fast_forward`；二者都只有 finalize 才结束 Session，其中 fast-forward 只移动 ref、不创建多余 Merge Commit；
- [ ] `merge.list` bounded/pageable 且只读取 session metadata；大量 unresolved conflict 的 Session 出现在 list 中时不能隐式 materialize/recompute 全部 conflict，调用 `merge.get` 才计算该单一 Session 的 status/unresolved；
- [ ] `merge.get` 在同一 SQLite read snapshot 内读取 revision/resolution 并计算 status/unresolved，不返回跨 revision 混合结果；`merge.list` 是 live operational inventory 而不是 pinned history traversal，并发 start/finalize/abort 允许影响后续 page，重新从首屏枚举可获得新的当前 inventory；
- [ ] criss-cross history 使用 deterministic virtual base，并且 ambiguous base slot 不发生错误 auto-merge；
- [ ] same-value changes auto merge；
- [ ] property conflict 返回完整 base/ours/theirs 且 Session 创建/分页/resolve 全程不写 Commit/ref；
- [ ] delete-vs-modify、relationship endpoint dependency、schema conflict、constraint conflict fixtures 通过；
- [ ] 大量 conflict 使用 bounded `merge.conflicts` page；分页 cursor 固定 session+revision，resolve 后旧 cursor 返回 `MERGE_SESSION_CHANGED`，不要求一次 materialize 全部 conflict；
- [ ] `merge.resolve` 可以跨多个调用逐步解决 conflict；一批 resolution 原子应用并仅递增一次 revision，在当前 revision 上重复设置相同值是 no-op，stale expectedRevision 即使 payload 相同也返回 `MERGE_SESSION_CHANGED`；已有 resolution 可以用同 conflictId 替换；
- [ ] resolve no-op 返回 revision R 与该 proposed state 的 status/unresolved；存在有效 resolution change 才返回 R+1，不能因为 API 调用本身无条件推进 revision；
- [ ] 已保存 resolution 对应的 conflict 因其它选择暂时消失时成为 dormant，不出现在当前 conflict inventory、不影响 candidate/unresolved；同一 deterministic conflictId 重现时恢复原 resolution，不同 conflict 不得误复用 dormant resolution；
- [ ] `merge.resolve` 对大 conflict set 先在 read snapshot 验证 requested batch、应用 proposed resolutions 并计算写入成功后 revision/status/unresolved，再在短 writer boundary 内重新 CAS revision 后写入；writer boundary 内不能重新扫描完整 conflict set；和 concurrent resolve/abort/finalize 竞争时 stale caller 只能得到 `MERGE_SESSION_CHANGED/NOT_FOUND`，不能覆盖较新的 resolution；
- [ ] unresolved conflict > 0 时 candidate query / finalize 返回 `MERGE_CONFLICT`；全部解决后 candidate 可通过 `options.mergeSession={id,revision}` 用 read/Search/Schema SHOW 检查且 `summary.commit=null`；mutation/version/LOAD CSV 不能借 candidate context 产生副作用；
- [ ] candidate mapping 正确：`up_to_date` candidate=ours，`fast_forward` candidate=theirs，diverged `ready` candidate=resolved merged state；`conflicted` 不存在完整 candidate；
- [ ] candidate query 在同一 SQLite read snapshot 内校验 requested revision 并 pin resolution set；query 开始后的并发 `merge.resolve` 不改变本 execution candidate，下一次仍使用旧 revision 的 query 返回 `MERGE_SESSION_CHANGED`；
- [ ] candidate 在 revision R 上通过多次 read 后，如果 resolution 被并发修改为 R+1，`merge.finalize(session,R)` 返回 `MERGE_SESSION_CHANGED` 且无 Commit/ref move；
- [ ] `merge.finalize` 的完整 candidate/delta/Schema/Constraint validation 先在 expected revision 的 read snapshot prepare；随后进入短 SQLite writer boundary，并在该边界内部重新校验 session revision/unresolved 与 target Branch head，再安装 prepared canonical/derived result；不得把大型 merge/conflict scan 放在 writer 内，也不得存在 check-revision/check-head 后再取锁的 TOCTOU，和并发 resolve/abort/finalize/GC/Branch write 竞争时最多一个合法 canonical outcome；
- [ ] candidate revision 未变但 target Branch 被删除时 finalize 返回 `BRANCH_NOT_FOUND`、head 已离开 pinned ours 时返回 `BRANCH_HEAD_MOVED`，两者都保留 Session；source Branch 后续移动不改变 pinned theirs；
- [ ] resolved diverged merge finalize 创建 parent1=target、parent2=source 的唯一 Merge Commit；Commit/ref move 与 Session/resolution 删除原子完成；
- [ ] finalize success 的 `commit` 恒为最终 resolved Snapshot：up_to_date=ours、fast_forward=theirs、merged=新 Merge Commit；
- [ ] `merge.finalize` 成功（包括 up_to_date/fast_forward/merged）或 `merge.abort(session, expectedRevision)` 成功后对应 Session/resolution 不再可读；stale abort 不能删除较新 revision 的工作；finalize/abort 的 SQLite/resource/I/O failure 整体 rollback，Session 仍保持可恢复且不得出现“canonical action 成功但 workspace 还可重复 finalize”或相反状态；
- [ ] merge snapshot 只用 first-parent + merge layer 可重建；
- [ ] open Merge Session survive connection close/process restart，`merge.get/list` 可恢复 resolution 进度；abort 后 workspace 消失且历史/ref 不变；
- [ ] open Merge Session 保护 pinned ours/theirs history 不被 GC；finalize/abort 后如果再无 Branch/Tag/其它 open Session root 才允许后续 GC 回收；
...
- [ ] explicit GC 不删除 Branch/Tag/open-Merge-Session reachable history，并随真正被删除的 Commit 清理其 Commit Data；
- [ ] reset 后 unreachable history 默认保留直到 explicit GC；
- [ ] restart 后 Commit DAG、Branch/Tag refs、Commit Data 与 open Merge Session/resolution 完整。

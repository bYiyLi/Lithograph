...
- `docs/development/cypher25-compatibility.md` 保留 Phase 00–12 已执行证据；Full-text family 与 Phase 12 provider supplemental inventory 均已闭合为 `done`。Phase 13 的 `db.index.semantic.*` 是 Lithograph procedure extension，不改变冻结 Cypher 语言 coverage 分母；当前 supplemental inventory 已同步本地完成证据，并明确保留跨平台 hosted artifact acceptance 为 `in_progress`。
...
| [13 Managed Semantic Vector / Embedding Provider](phases/13-managed-semantic-vector.md) | `in_progress` | 保留 Raw Vector；新增 SQLite Embedding Provider、Semantic Index、format4 Embedding cache、文本 query/rebuild/cache maintenance | 01、07–12 |

...
- **先执行 runtime feasibility gate，再重构 write/transaction program**：用真实 loadable extension / 最小 targeted probe 在最低 SQLite 3.45.0 与 current runtime 上确认三件独立宿主事实：① eponymous `lithograph_rows()` virtual-table cursor 可以在同一个 host `sqlite3*` 上于第一次 runtime side effect 时建立普通 write SAVEPOINT、跨多次 `xNext` 保持该 boundary、写 `main`，并在 success terminal release、error/interrupt/`xClose` early-stop 时正确 rollback/release；② stock SQLite 对 virtual-table `xClose` 返回码的真实传播行为，以及 cleanup failure 必须采用的可观察 recovery channel；③ 处于 autocommit mode 的 scalar callback 与 rows cursor 都可以在同一个 host connection 内执行 transaction-owning query所需的连续 inner `BEGIN/COMMIT/ROLLBACK` boundary。三项都不得依赖辅助 graph connection、SQLite fork、未公开 API或隐藏 Native query path；同时覆盖 outer statement teardown / connection usability。已执行的 3.45.0 / 3.51.0 probe 证明①③可行，同时证明 VDBE 丢弃 `xClose` 返回值，②不能按原假设通过；因此先按 Interfaces 真源把该失败语义收敛为 full-rollback + connection cleanup-failure quarantine，再继续依赖该结果的 rows-write implementation；
...
- 当前 rusqlite vtab wrapper 的 `xClose` 通过 drop cursor并固定返回 `SQLITE_OK`；而 stock SQLite VDBE 本身同样丢弃 module `xClose` 返回码，因此不能通过 FFI shim创造 closing-statement error channel。rows close path 必须在 wrapper 的实际 xClose/drop dispatch 中显式执行 execution cancel/rollback：正常 cleanup 成功后释放；rollback/release cleanup failure触发 full `ROLLBACK` fallback并把 connection 标记为 cleanup-failure quarantine，后续任一 Lithograph entrypoint 在工作前返回 `INTERNAL_ERROR` 并要求丢弃 connection。不得因为 host 不传播 `xClose` rc 就静默遗忘 cleanup failure；
...
- Large read：release-profile 10,000,000 rows 完整消费，first row 127,534,320 µs，full consume 384,597,019 µs，peak RSS 26,066,944 bytes。
- Transaction upstream / external I/O：fresh format3 DB 上 `LOAD CSV` 10,000,000 rows + `IN TRANSACTIONS OF 4096 ROWS`，first row 104,878 µs，full consume 238,825,361 µs，peak RSS 45,924,352 bytes。
- Mutating RETURN：1,000,000-row fast RETURN peak RSS 1,112,391,680 bytes，与无 RETURN baseline 1,112,096,768 bytes 同阶且仅差约 0.03%；不存在第二份 full-result retained collection。100,000-row fast/slow writer-hold 分别 30,983,172 / 125,870,130 µs，slow consumer 明确施加 backpressure；early-close 只观察 1 row、无 summary，并 rollback unpublished write。
- TEMP/disk：真实 Core `BindingSpill` 强制 `temp_store=FILE`；4096×1 KiB binding test 记录 5,615,616 logical TEMP bytes。
...
- rows `xClose` / wrapper drop dispatch 是否真的执行 fail-closed cleanup；stock SQLite 吞掉 `xClose` rc 后，rollback/release failure是否进入 full-rollback + connection quarantine，而不是随 cursor Drop 一起丢失；

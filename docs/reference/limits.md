...
**版本：v0.1.1。** 本页列出应用可依赖的入口约束，不把内部 cache budget、性能实测或机器容量当作通用硬上限。
...
v0.1.1 读取支持格式 1–3，新建使用 3。旧格式写入前需要显式 init 迁移；无法将 format 3 自动降级。内部 `_lithograph_*` namespace 保留，不插入、删除、重命名或加 trigger/index。

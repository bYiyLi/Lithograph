...
Phase 07 的 20 个 targeted scenarios 覆盖 canonical versioned Schema、Graph Type SET/ADD/ALTER/DROP 与 identifying/implied endpoint semantics、standalone/dependent Constraint、existing-data/write-time validation、DDL conflict/IF EXISTS/IF NOT EXISTS、Branch/time-travel Schema isolation、persistent property type lowering、`SHOW CURRENT GRAPH TYPE` canonical round-trip/virtual graph、`SHOW CONSTRAINTS` type filter/classification，以及 node/relationship lookup/range/text/point standard index。Range regression 锁定 Integer/Float 数值等价、`2^53` 精度边界、`IN`、inequality、STRING prefix 与 temporal ordering；Point regression 锁定 Cartesian/WGS-84 bbox/distance、antimeridian 与 Graph View hidden-candidate filtering。derived cache 删除后可从 canonical Snapshot 重建，historical planning 只读取目标 Commit 的 Schema definition。
